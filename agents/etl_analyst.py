...
def transform_load_tool(input_file_path: str, output_folder:str, output_format: str, user_question: str) -> str:
    ...
    return (
            f"The data is transformed and saved at {output_folder} "
...
def llm_node(state: ETLAgentSchema):
    ...
    system_message = SystemMessage(
        ...
        Use the appropriate tool when the user asks you to:
        - extract and load data
        - transform and load data

        If a tool has completed the requested operation successfully,
        inform the user of the result and do not call the tool again.
        """
    )
...
if __name__ == "__main__":
    ...
    #     {"messages":[HumanMessage(content="I want to extract the data from the API endpoint 'https://pokeapi.co/api/v2/pokemon' and save it to data/extract folder in csv format")]}
    # )
    ...
    response = etl_analyst.invoke(
         {"messages":[HumanMessage(content=f"""
            ...
            and save the transformed data in the 'data\\transform' folder in the csv format.
